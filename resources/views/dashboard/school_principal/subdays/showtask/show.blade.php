...
<div class="container-fluid">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        ...
        <h2>{{ $subday->school->name }} Principal: {{ $user->name }}</h2>
        <h3>{{ $subday->school->name }} Teacher: {{ $subday->schoolTeacher->myself->name }}</h3>
        <h3>Substitute Teacher: {{ $subday->substituteTeacher->myself->name }}</h3>
        <p>From: {{ Carbon\Carbon::parse($subday->absent_day_day_start)->toFormattedDateString() }}</p>
        <p>To: {{ Carbon\Carbon::parse($subday->absent_day_day_end)->toFormattedDateString() }}</p>
        <p>{{ $subday->part_of_day }}</p>
        <h3>Task Items</h3>
