...
<div class="container-fluid">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        ...
        <h3>{{ $teacher->school->name }} Administrator: {{ $user->name }}</h3>
        <h4>Principal: {{ $teacher->school->principal->myself->name }}</h4>
        <h2 class="sub-header">Book: {{ $subTeacher->myself->name }}</h2>
        <form role="form" method="POST" action="{{ URL::route('store-sub_day-school_administrator') }}">
            {{ csrf_field() }}
            @if (count($errors) > 0)
